...
def evaluate(model, dataloader, device, threshold=0.5, num_coarse=3):
    ...
    for batch in tqdm(dataloader, desc="Eval", leave=False):
        ...
        coarse_probs.append(torch.softmax(outputs["logits_coarse"], dim=-1).cpu().numpy())
        coarse_labels.append(batch["label_coarse"].cpu().numpy())
        fine_probs.append(torch.sigmoid(outputs["logits_fine"]).cpu().numpy())
        fine_labels.append(batch["label_fine"].cpu().numpy())

    coarse_metrics = compute_coarse_metrics(np.concatenate(coarse_labels), np.concatenate(coarse_probs))
    fine_metrics = compute_multi_label_metrics(np.concatenate(fine_labels), np.concatenate(fine_probs), threshold)
