...
def compute_coarse_metrics(y_true, y_prob) -> Dict[str, Any]:
    """Evaluate the single-label coarse task and retain LRAP used in the original experiments."""
...
def compute_multi_label_metrics(y_true, y_prob, threshold: float = 0.5) -> Dict[str, Any]:
    """Evaluate the fine-grained multi-label task."""
    y_true = np.asarray(y_true).astype(int)
    y_prob = np.asarray(y_prob).astype(float)
